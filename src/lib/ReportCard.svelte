  ...
  <div class="card-body flex flex-col gap-4">
    <div class="flex justify-between items-center">
      <div class="text-sm opacity-75">
        Submitted by <span class="font-semibold">@{$profileDisplay}</span> on {formatTimestamp(event.created_at)}
      </div>
      <div class="px-2 py-1 bg-primary text-primary-content rounded-full">
        {reason}
      </div>
    </div>
    {#if etag}
    ...
    <div class="card-actions justify-end">
      <div class="dropdown dropdown-end">
        <div tabindex="0" role="button" class="btn btn-sm">Actions <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></div>
